...
export const CourseCard: React.FC<CourseCardProps> = ({ course }) => {
    ...
    return (
        <div className="bg-white group relative overflow-hidden rounded-[2.5rem] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-gray-100">
            {/* Category Badge */}
            <div className={`absolute top-5 right-5 z-10 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${isDharmik ? 'bg-orange/10 text-orange-dark' : 'bg-amber-50 text-amber-800'}`}>
                {course.level}
            </div>
            ...
            <div className="aspect-[4/3] overflow-hidden bg-gray-50 flex items-center justify-center relative">
                {!imageError ? (
                    ...
                    <div className={`w-full h-full flex flex-col items-center justify-center gap-2 ${isDharmik ? 'bg-orange/10' : 'bg-amber-50'}`}>
                        <svg className={`w-12 h-12 ${isDharmik ? 'text-orange/40' : 'text-amber-200'}`} fill="currentColor" viewBox="0 0 24 24"><path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z" /></svg>
